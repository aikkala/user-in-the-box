...
def check_study_dataset_dir(DIRNAME_STUDY):
    if not os.path.exists(DIRNAME_STUDY):
        download_datasets = input(
            "Could not find reference to the ISO-VR Dataset. Do you want to download it (~3.9GB after unpacking)? (y/N) ")
        if download_datasets.lower().startswith("y"):
            ...
            resp = open("/home/florian/mpc-mujoco-git/mpc_mujoco/data/ISO_VR_Dataset.zip", 'rb')  #urlopen("http://zenodo.org/ISO_VR_Dataset.zip")
            ...
            print("done.")
            assert os.path.exists(DIRNAME_STUDY), "Internal Error during unpacking of ISO-VR Dataset."
        else:
            raise FileNotFoundError("Please ensure that 'DIRNAME_STUDY' points to a valid directory containing the ISO-VR Dataset.")
