...
class RemoteDriving(BaseTask):
  ...
  def __init__(self, sim, **kwargs):
    ...
    sim.model.geom_size[self.target_plane_geom_idx] = \
    ...
    sim.model.cam_pos[sim.model.camera_name2id('for_testing')] = np.array([-2, 0, 2.5])
    sim.model.cam_quat[sim.model.camera_name2id('for_testing')] = \
      sim.model.body_quat[sim.model.body_name2id("eye")].copy()
  ...
  def update_car_dynamics(self, sim):
    """
    ...
    """
    throttle_control = sim.data.qpos[sim.model._joint_name2id[self.joystick_joint]]
    throttle = throttle_control * self.throttle_factor
    sim.data.qfrc_applied[sim.model._joint_name2id[self.engine_back_joint]] = throttle
    sim.data.qfrc_applied[sim.model._joint_name2id[self.engine_front_joint]] = throttle
  ...
  def update(self, sim):
    ...
    # Update car dynamics
    self.update_car_dynamics(sim)

    # Distance between car and target
    car_target_contact = [contact for contact in sim.data.contact if
                          "target" in {sim.model.geom_id2name(contact.geom1), sim.model.geom_id2name(contact.geom2)}
                          and any([f"wheel{i}" in {sim.model.geom_id2name(contact.geom1),
                                                   sim.model.geom_id2name(contact.geom2)} for i in range(1, 5)])]
    contacts = [{sim.model.geom_id2name(contact.geom1), sim.model.geom_id2name(contact.geom2), contact.dist}
    ...
    fingertip_joystick_contact = [contact for contact in sim.data.contact if
                                  self.joystick_geom in {sim.model.geom_id2name(contact.geom1),
    ...
    if info["inside_target"] and np.abs(sim.data.get_body_xvelp("car")[1]) <= self.car_velocity_threshold:
      finished = True
  ...
  def spawn_car(self, sim, rng):
    # Choose qpos value of slide joint in x-direction uniformly from joint angle range
    new_car_qpos = rng.uniform(*sim.model.jnt_range[sim.model._joint_name2id[self.car_joint]])

    sim.data.qpos[sim.model._joint_name2id[self.car_joint]] = new_car_qpos
    sim.forward()

    # Get car position
    self.car_position = sim.data.get_body_xpos(self.car_body).copy()

  def spawn_target(self, sim, rng):
    ...
    sim.model.geom_size[sim.model._geom_name2id["target"]][0] = self.target_halfsize
