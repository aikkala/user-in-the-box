...
class Simulator(gym.Env):
  ...
  def __init__(self, simulator_folder, render_mode="rgb_array", render_show_depths=False, run_parameters=None):
    ...
    run_params = dict()
    run_params["action_sample_freq"] = self.config["llc"]["action_sample_freq"]

    
    if not hasattr(self, 'llc_model'):
        ...
        print(f'Loading model: {os.path.join(llccheckpoint_dir, "model_40000000_steps.zip")}\n')
        self.llc_model = PPO.load(os.path.join(llccheckpoint_dir, "model_40000000_steps.zip"))
    

    self._max_steps = run_params["action_sample_freq"]
    self._dwell_threshold = int(0.5*run_params["action_sample_freq"])
  ...
  def _initialise_action_space(self):
    bm_jnt_range = np.ones((2,2)) * np.array([-1.0, 1.0])
  ...
  def step(self, action):
    ...
    """
    action = (action + 1) * (0.3 - (-0.3)) / 2 + (-0.3)
    action = np.append(np.array([0]), action)
    self._steps = 0
    acc_reward = 0
    
    
    llc_action, _states = self.llc_model.predict(self.get_llcobservation(action), deterministic=True)
  ...
  def get_llcobservation(self,action):
    ...
    stateful_information = self.task._target_origin + action
    if stateful_information is not None:
      observation["stateful_information"] = stateful_information
