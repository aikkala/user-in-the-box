...
from UIB.envs.mobl_arms.driving.reward_functions import NegativeExpDistanceWithHitBonus, RewardJoystick
...
class DrivingEnv(FixedEye):
  ...
  def __init__(self, **kwargs):
    ...
    include_car_scene = ET.Element('include', file='car.xml')
    root.append(include_car_scene)

    # Save the modified XML file and replace old one
    xml_file = os.path.join(project_path(), 'envs/mobl_arms/models/variants/driving_env.xml')
  ...
  def step(self, action):
    ...
    car_target_contact = [contact for contact in self.sim.data.contact if "target" in {contact.geom1, contact.geom2} and any([f"wheel{i}" in {contact.geom1, contact.geom2} for i in range(1, 5)])]
    ...
    reward += self.reward_function_joystick.get(self, body_at_joystick)
